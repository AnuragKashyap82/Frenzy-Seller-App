...
public class AdapterReturnedOrder extends RecyclerView.Adapter<AdapterReturnedOrder.HolderReturnedProducts> {

    private Context context;
    private ArrayList<ModelAllOrders> allOrdersArrayList;

    public AdapterReturnedOrder(Context context, ArrayList<ModelAllOrders> allOrdersArrayList) {
    ...
    @NonNull
    @Override
    public HolderReturnedProducts onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.order_item_layout, parent, false);
        return new HolderReturnedProducts(view);
    }

    @Override
    public void onBindViewHolder(@NonNull HolderReturnedProducts holder, int position) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("BUTTON_CODE", "RETURNED_ORDER");
                context.startActivity(intent);
            }
        });
    }
    private void loadOrderDetails(String orderBy, String orderId, HolderReturnedProducts holder) {
        ...
        documentReference.collection("myOrders").document(orderId)
                .addSnapshotListener(new EventListener<DocumentSnapshot>() {
                    ...
                    public void onEvent(@Nullable DocumentSnapshot snapshot, @Nullable FirebaseFirestoreException error) {
                        ...
                        String returnedDate = snapshot.get("orderReturnDate").toString();
                        String returnedTime = snapshot.get("orderReturnTime").toString();

                        holder.orderDate.setText("Order Date: "+orderDate);
                        holder.deliveryWithinTv.setText("Returned: "+ returnedDate+" - "+returnedTime);
                        holder.deliveryWithinTv.setTextColor(context.getResources().getColor(R.color.colorPrimary));
                    }
                });
    }

    private void loadProductDetails(String productId, HolderReturnedProducts holder) {
    ...
    public class HolderReturnedProducts extends RecyclerView.ViewHolder {

        private ImageView productImage, orderIndicator;
        private TextView productTitle, deliveryWithinTv, orderDate;

        public HolderReturnedProducts(@NonNull View itemView) {
            super(itemView);
