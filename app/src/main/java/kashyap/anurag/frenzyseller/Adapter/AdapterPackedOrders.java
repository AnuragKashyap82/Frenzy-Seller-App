...
public class AdapterPackedOrders extends RecyclerView.Adapter<AdapterPackedOrders.HolderPackedOrders> {

    private Context context;
    private ArrayList<ModelAllOrders> allOrdersArrayList;

    public AdapterPackedOrders(Context context, ArrayList<ModelAllOrders> allOrdersArrayList) {
    ...
    @NonNull
    @Override
    public HolderPackedOrders onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.order_item_layout, parent, false);
        return new HolderPackedOrders(view);
    }

    @Override
    public void onBindViewHolder(@NonNull HolderPackedOrders holder, int position) {
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.putExtra("BUTTON_CODE", "PACKED_ORDER");
                context.startActivity(intent);
            }
        });
    }
    private void loadOrderDetails(String orderBy, String orderId, HolderPackedOrders holder) {
        ...
        documentReference.collection("myOrders").document(orderId)
                .addSnapshotListener(new EventListener<DocumentSnapshot>() {
                    ...
                    public void onEvent(@Nullable DocumentSnapshot snapshot, @Nullable FirebaseFirestoreException error) {
                        ...
                        String packedDate = snapshot.get("orderPackedDate").toString();
                        String packedTime = snapshot.get("orderPackedTime").toString();

                        holder.orderDate.setText("Order Date: "+orderDate);
                        holder.deliveryWithinTv.setText("Packed on: "+ packedTime+" - "+packedDate);
                        holder.deliveryWithinTv.setTextColor(context.getResources().getColor(R.color.gray02));
                    }
                });
    }

    private void loadProductDetails(String productId, HolderPackedOrders holder) {
    ...
    public class HolderPackedOrders extends RecyclerView.ViewHolder {

        private ImageView productImage, orderIndicator;
        private TextView productTitle, deliveryWithinTv, orderDate;

        public HolderPackedOrders(@NonNull View itemView) {
            super(itemView);
